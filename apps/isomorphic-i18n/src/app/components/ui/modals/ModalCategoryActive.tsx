...
export default function ModalCategoryActive({
  title,
  ...
  const text = {
    inactiveRequest: lang === 'ar' ? 'تفعيل القسم' : 'Activate Category',
  ...
  const handleActivateRequest = async () => {
    try {
      ...
      const response = await axiosClient.patch(`/api/Category/ChangeActivationStatus/${userId}`, null, {
        headers: {
