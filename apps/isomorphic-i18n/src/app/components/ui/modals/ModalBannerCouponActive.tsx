...
type UpdatePlanFormProps = {
  ...
  lang: string;
  userId?: string;
  initialresolveNote?: string;
  status: string;
};

export default function ModalBannerCouponActive({
  title,
  modalBtnLabel = 'تعديل الخطة',
  onSuccess,
  userId,
  initialresolveNote,
  status,
  ...
  const text = {
    inactiveRequest: lang === 'ar' ? 'تفعيل الفرع' : 'Activate Branch',
    cancel: lang === 'ar' ? 'إلغاء' : 'Cancel',
    deactivate: lang === 'ar' ? 'إلغاء التفعيل' : 'Deactivate',
    activeRequst: lang === 'ar' ? 'إلغاء تفعيل الفرع' : 'Deactivate Branch',
  ...
  const handleDeclineRequest = async () => {
    try {
      ...
      const response = await axiosClient.patch(`/api/Branch/DeActivate/${userId}`, null, {
        headers: {
      ...
      if (response.status === 200) {
        toast.success(lang === 'ar' ? 'لقد اصبح الفرع غير نشط' : 'You have become an inactive branch.');
  ...
  const handleActivateRequest = async () => {
    try {
      ...
      const response = await axiosClient.patch(`/api/Branch/Activate/${userId}`, null, {
        headers: {
          'Accept-Language': lang,
        },
      });
      ...
      if (response.status === 200) {
        toast.success(lang === 'ar' ? 'تم تفعيل الفرع' : 'Branch activated successfully');
        setBranchesData(true);
        closeModal();
      } else {
