...
export default function ChooseBranchDelivery({ lang = 'en', initialCurrencyAbbreviation,branches, orderId, pageHeader, initialDeliveryInfo, initialOrder, initialLocationDirection }: ChooseDeliveryFormProps) {
  ...
  const { setOrderDetailsStatus, setOrderDetailsTable, orderDetailsStatus, mainBranch } = useUserContext();  
  ...
  const fetchDrivers = async (branchId: string, pageNumber = 1) => {
    ...
    try {
      ...
      // .filter((driver: any) => driver.deliveryStatus === 1);
  ...
  useEffect(() => {
    fetchDrivers(mainBranch, 1);
    getOrderAndDelivery();
    setPage(1);
  }, [mainBranch, orderDetailsStatus, lang]);
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (bottom && page < maxPage) {
        ...
        setPage(nextPage);
        fetchDrivers(mainBranch, nextPage);
        getOrderAndDelivery();
      }
    };
                ...
                <div className="w-full bg-gray-50 rounded-lg p-3 flex flex-col gap-2">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-green-600 text-md flex gap-2">
                      {(order.status === 3 || order.status === 4)? order.shippingFees : order.shippingFees} {currencyAbbreviation === "ر.س" ? (<Image src={sarIcon} alt="SAR" width={16} height={16} />) : (<span>{currencyAbbreviation}</span>)}
                    </span>
                  </div>
                </div>                
            ...
            {drivers.length == 0?
              (
                <div className='text-center'>
                  <Empty /> <Text className="mt-3">{lang === 'ar' ? 'لا توجد سائقين' : 'No Drivers'}</Text>
                </div>
              )
              :
              ...
              <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 3xl:grid-cols-4 gap-4 p-0`}>
                {drivers.map(driver => (
                  <div key={driver.id} className='w-full bg-white rounded-[5px] p-2 flex flex-col justify-between gap-4'>
                    <div className='w-full flex justify-between items-center gap-2'>
                      <div className='w-[calc(100%-48px)] flex gap-2'>
                        <Image
                          src={driver.personalPhotoUrl || delivery.src}
                        ...
                        <div className='w-[calc(100%-38px)]'>
                          <div className='w-full flex gap-1'>
                            ...
                            <p className="font-semibold text-[14px] text-[#AEAEAE] truncate overflow-hidden whitespace-nowrap max-w-[80px]">
                              ({driver.tripCount} {text.trip})
                            </p>
                          </div>
                          <p className="font-semibold text-[14px] capitalize text-[#979797]">
                            {text.away} : <span>{driver.distance || 50}</span>
                          </p>
                          {/* {formatDuration(driver.routeDurationToBranch, lang) && (
                            <p className="font-semibold text-[14px] capitalize text-[#979797]">
                              {text.remainingTime} : <span>{formatDuration(driver.routeDurationToBranch, lang)}</span>
                            </p>
                          )} */}
                        </div>
                      </div>
                    ...
                    <div className='w-full mt-3'>
                      ...
                      <div className="w-full flex flex-row gap-4 mt-3">
                        <ActionButton
                          text={text.hiring}
                          type="submit"
                          disabled={submittingDriverId === driver.id}
                          buttonClassName={`p-1 w-full ${submittingDriverId === driver.id ? 'bg-[#cacaca] cursor-default' : 'bg-green hover:bg-green-dark'}`}
                          textClassName="text-white"
                          onClick={()=>assignDriver(driver.id)}
                          // onClick={() => acceptDeliveryOffer(driver.id)}
                        />
                        <RoleExist PageRoles={['GetDeliverById']}>
                          <Link
