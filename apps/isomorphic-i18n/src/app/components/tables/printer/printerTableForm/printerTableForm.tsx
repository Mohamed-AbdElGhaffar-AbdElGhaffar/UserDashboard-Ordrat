...
export default function PrinterTableForm({
  title,
  onSuccess,
  lang = 'en',
}: TableFormProps) {
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await axiosClient.post(
          `/api/Printer/CreatePrinter/${mainBranch}`,
          formData,
          {
            headers: {
        ...
        toast.success(
          lang === 'ar' ? 'تم انشاء الطابعة بنجاح!' : 'Printer created successfully!'
        );
        setIsSubmit(false);
        closeModal();
        ...
        if (axiosError.response && axiosError.response.status === 401) {
          ...
          toast.error(
            lang === 'ar'
              ? 'فشل في انشاء الطابعة. حاول مجددا.'
              : 'Failed to create printer. Please try again.'
          );
        }
      }
    },
      ...
      <div className={`m-auto ps-3 rounded-xl pe-1.5 me-1.5 pb-4 pt-4 IBM-Plex-sans ${styles.customScroll}`}>
        ...
        <form onSubmit={(e) => {
          ...
          <div className="flex flex-col gap-4">
            <Input label={text.name} placeholder={text.name} name="name" value={mainFormik.values.name} onChange={mainFormik.handleChange} onBlur={mainFormik.handleBlur} error={mainFormik.touched.name && mainFormik.errors.name ? mainFormik.errors.name : ''} className="input-placeholder text-[16px]" inputClassName='text-[16px]' />
            <Input label={text.IP} placeholder={text.IP} name="IP" value={mainFormik.values.IP} onChange={mainFormik.handleChange} onBlur={mainFormik.handleBlur} error={mainFormik.touched.IP && mainFormik.errors.IP ? mainFormik.errors.IP : ''} className="input-placeholder text-[16px]" inputClassName='text-[16px]' />
          </div>
