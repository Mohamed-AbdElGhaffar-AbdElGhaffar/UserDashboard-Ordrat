...
export default function RoleCard({
  ...
  const text = {
    ...
    renameRole: lang === 'ar' ? 'إعادة تسمية المجموعة' : 'Rename',
    removeRole: lang === 'ar' ? 'حذف المجموعة' : 'Remove Group',
    total: lang === 'ar' ? `إجمالي` : `Total`,
    Users: lang === 'ar' ? `مستخدم` : `users`,
    editRole: lang === 'ar' ? 'رؤية الصلاحيات' : 'View Permissions',
  };
  
  const { openModal } = useModal();
  return (
    <div className={cn('rounded-lg border border-muted p-6 bg-white', className)}>
      ...
      {/* {users &&( */}
        <div className="mt-4 flex items-center gap-2">
          <div className="flex items-center">
            {Array.from({ length: Math.min(users, 4) }).map((_, index) => (
              <figure
                key={index}
          ...
          <span>{text.total} {users} {text.Users}</span>
        </div>
      {/* )} */}
      <RoleExist PageRoles={['ViewRoles']}>
        <ModalButton
          customSize="700px"
          variant="outline"
          label={text.editRole}
          icon={<UserCog className="h-5 w-5" />}
          view={<ViewRoles lang={lang} roles={roles}/>}
