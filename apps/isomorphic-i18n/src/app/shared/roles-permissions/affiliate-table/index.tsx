...
import { Affiliate } from '@/data/users-data';
...
const FilterElement = dynamic(
  () => import('@/app/shared/roles-permissions/affiliate-table/filter-element'),
  { ssr: false }
);
...
export default function AffiliateTable({ usersData = [], lang }: { usersData: any[]; lang:string; }) {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosClient.get(`/api/Affiliate/wallet`);

      console.log("response.data.transactions: ", response.data.transactions);

      const apiData = response.data;

      const transformedUsers: Affiliate[] = apiData.transactions.map((user: any) => ({
