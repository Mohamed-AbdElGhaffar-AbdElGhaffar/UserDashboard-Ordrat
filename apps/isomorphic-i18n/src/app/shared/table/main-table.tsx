...
export default function MainTable<TData extends Record<string, any>>({
  ...
  components,
  lang = 'en',
}: MainTableProps<TData> & { lang: string }) {
    ...
    <>
      ...
      {isEmpty(table.getRowModel().rows) && (
        <div className="py-5 text-center lg:py-8">
          <Empty /> <Text className="mt-3">{lang === 'ar' ? 'لا توجد بيانات' : 'No Data'}</Text>
        </div>
      )}
    </>
  );
...
export function TableHeadBasic<TData extends Record<string, any>>({
  ...
  isRightScrollable,
  lang = 'en',
}: CustomHeaderCellProps<TData> & { lang: string }) {
          ...
          >
            ...
            {header.column.getCanSort() ? (
              <button
                ...
                className="ms-2"
                aria-label={lang === 'ar' ? 'ترتيب حسب العمود' : 'Sort by column'}
              >
                {{
