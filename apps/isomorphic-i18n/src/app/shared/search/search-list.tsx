...
export default function SearchList({ onClose ,lang }: { onClose?: () => void |any ,lang:string } ) {
  const inputRef = useRef(null);
  const [searchText, setSearchText] = useState('');
  const { t } = useTranslation(lang!, "nav");
  const links = PageLinks({ lang: lang });
    ...
    <>
      <div className="flex items-center px-5 py-4 bg-white dark:bg-black ">
        <Input
          ...
          placeholder={lang ==='ar'? 'يمكنك البحث عن الصفحات':'Search pages here'}
        ...
        <>
          {menuItemsFiltered.length === 0 ? (
            <Empty
              ...
              text={lang==='ar'? "لا يوجد نتيجة":"No Result Found"}
          ...
          return (
            <Fragment key={item.name + '-' + index}>
              {item?.href ? (
                <Link
                lang={lang!}
                  href={`/${lang!}${item?.href}` as string}
                  className="relative my-0.5 flex items-center rounded-lg px-3 py-2 text-sm hover:bg-red-50 focus:outline-none focus-visible:bg-gray-100 dark:hover:bg-gray-50/50 dark:hover:backdrop-blur-lg"
                >
